...
int strategyHiLo(int a, int b) { return a - b; }

int partition(int *array, int lowTarget, int highTarget,
              int (*strategyCmp)(int, int)) {
  int pivot = array[(highTarget + lowTarget) / 2];
  int low = lowTarget;
  int high = highTarget;
  while (1) {

    while (strategyCmp(array[low], pivot) < 0)
      low--;
    while (strategyCmp(array[high], pivot) > 0)
      high--;
    if (low >= high)
      return high;
...
int partitionStr(char **arr, int lowTarget, int highTarget,
                 int (*cmp_func)(const char *a, const char *b)) {
  char *pivot = arr[(highTarget + lowTarget) / 2];
  int low = lowTarget;
  int high = highTarget;
  while (1) {
    while (cmp_func(arr[low], pivot) < 0)
      low++;
    while (cmp_func(arr[high], pivot) > 0)
      high--;
    if (low >= high)
      return high;
